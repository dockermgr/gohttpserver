...
# @@File             :  install.sh
# @@Description      :  Container installer script for gohttpserver
...
REPO="${DOCKERMGRREPO:-https://github.com/$SCRIPTS_PREFIX}/gohttpserver"
APPVERSION="$(__appversion "$REPO/raw/$REPO_BRANCH/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="gohttpserver"
export INSTDIR="$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/gohttpserver"
export DOCKERMGR_CONFIG_DIR="${DOCKERMGR_CONFIG_DIR:-$HOME/.config/myscripts/$SCRIPTS_PREFIX}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the mountpoint directory
export APPDIR="$HOME/.local/share/srv/docker/gohttpserver"
...
HUB_IMAGE_URL="casjaysdevdocker/gohttpserver"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [gohttpserver.$SET_HOST_FULL_NAME] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
if [ -n "$GOHTTPSERVER_USERNAME" ]; then
  CONTAINER_USER_NAME="$GOHTTPSERVER_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${GOHTTPSERVER_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$GOHTTPSERVER_PASSWORD" ]; then
  CONTAINER_USER_PASS="$GOHTTPSERVER_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  if [ "$CONTAINER_USER_PASS" = "random" ]; then
    CONTAINER_USER_PASS="$(__password "${CONTAINER_PASS_LENGTH:-16}")"
  fi
  CONTAINER_USER_PASS="${GOHTTPSERVER_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
